...
class RealSenseDataSaver(Node):
    def __init__(self):
        ...
        self.demo_run = False  # Flag to check if demo.py has run
        self.grasp_processed = False  # Flag to indicate if Grasp ID 1 has been processed

        # Path to save images, CSV, etc.
        self.save_path = '/home/moniesha/graspnet-baseline/doc/example_data'
        self.csv_path = '/home/moniesha/graspnet-baseline/doc/top_50_grasp_poses.csv'

        # Ensure save directory exists
        os.makedirs(self.save_path, exist_ok=True)

        self.get_logger().info("RealSense Data Saver Node Initialized")
    ...
    def color_callback(self, msg):
        ...
        try:
            ...
            color_image_path = os.path.join(self.save_path, 'color.png')
    ...
    def depth_callback(self, msg):
        ...
        try:
            ...
            depth_image_path = os.path.join(self.save_path, 'depth.png')
    ...
    def create_meta_file(self):
        if self.intrinsic_matrix is not None:
            ...
            meta_file_path = os.path.join(self.save_path, 'meta.mat')
    ...
    def verify_and_run_demo(self):
        if self.demo_run or self.grasp_processed:
            return  # Skip demo if it has already run or Grasp ID 1 is processed
        ...
        if os.path.exists(color_image_path) and os.path.exists(depth_image_path) and os.path.exists(meta_file_path):
            self.get_logger().info("All required files are successfully saved. Proceeding to run demo.py.")
            demo_script_path = '/home/moniesha/graspnet-baseline/demo.py'
            checkpoint_path = '/home/moniesha/graspnet-baseline/checkpoint-rs.tar'

            try:
                subprocess.run(['python3', demo_script_path, '--checkpoint_path', checkpoint_path], check=True)
                self.get_logger().info("demo.py script executed successfully.")
                self.demo_run = True  # Mark demo as run
                self.grasp_processed = True  # Mark Grasp ID 1 as processed
            except subprocess.CalledProcessError as e:
                self.get_logger().error(f"Failed to run demo.py: {e}")
        else:
            self.get_logger().warn("Required files are not found. Skipping demo.py execution.")
