...
// Function to log time values to a CSV file

void log_time_value(
    long duration_ms,
    const std::string& filename = "/home/moniesha/Evaluation metrics/Stability/graspnet/timings.csv")
{
...
int main(int argc, char** argv)
{
  ...
  auto move_group_node = rclcpp::Node::make_shared("move_robot", node_options);
  ...
  // Create a publisher for the custom_gripper and sensor topic
  auto gripper_pub = move_group_node->create_publisher<std_msgs::msg::String>("custom_gripper", rclcpp::QoS(10));
  auto sensor_pub = move_group_node->create_publisher<std_msgs::msg::String>("relay_control", rclcpp::QoS(10));
  ...
  auto const attached_object = [frame_id =
    ...
    primitive.type = primitive.BOX;
    primitive.dimensions.resize(3);
    primitive.dimensions[primitive.BOX_X] = 0.090;  // 0.08 earlier
    primitive.dimensions[primitive.BOX_Y] = 0.030; // Size in meters
    primitive.dimensions[primitive.BOX_Z] = 0.221;// finray 0.221 (0.225 earlier), 0.180 (0.190 earlier) for rg2 baseline
  ...
  //target_pose_4 = Pre-drop pose s
  //Pre-drop pose for rg2 baseline gripper  
  /*target_pose_4.position.x = 0.2115;
  target_pose_4.position.y = 0.4500;
  target_pose_4.position.z = 0.5535;
  target_pose_4.orientation.x = 0;
  target_pose_4.orientation.y = 1;
  target_pose_4.orientation.z = 0;     
  target_pose_4.orientation.w = 0;

  //Pre-drop pose for rg2 fin-ray gripper 
  target_pose_4.position.x = 0.2090;
  target_pose_4.position.y = 0.5011;
  target_pose_4.position.z = 0.6092;
  target_pose_4.orientation.x = 0;
  target_pose_4.orientation.y = 1;
  target_pose_4.orientation.z = 0;     
  target_pose_4.orientation.w = 0;*/
  ...
  while (attempts <= 5) {
    GraspPoseWithScore grasp_data = convertAndPrintGraspDataToBaseLink("/home/moniesha/graspnet-baseline/doc/top_50_grasp_poses.csv", tf_buffer, attempts);  
    ...
    RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Received pose with score: %.4f", score_2a);
      ...
      double distance1 = 0.250;//Adjusting gripper length (other 0.170, with fruit 178) for rg2 baseline gripper, for fin-ray 0.250 (0.254 initial))
      ...
      if (success) {
          ...
          RCLCPP_INFO(LOGGER, "Published 'open' to custom_gripper topic. Waiting 2 seconds...");
          rclcpp::sleep_for(std::chrono::seconds(3));

          // ✅ Append attempt number and score to CSV file
          std::string output_file = "/home/moniesha/Evaluation metrics/grasp_quality_score/graspnet/success_scores.csv";
  ...
  if (planAndExecuteCartesianPath(move_group, start_pose_2, target_pose_2, PLANNING_GROUP)) {
    ...
    // Publish "close" to custom_gripper topic and wait for 5 seconds
        ...
        gripper_pub->publish(gripper_msg);
        RCLCPP_INFO(LOGGER, "Published 'close' to custom_gripper topic. Waiting 2 seconds...");
        rclcpp::sleep_for(std::chrono::seconds(3));
  ...
  if (planAndExecuteCartesianPath(move_group, start_pose_3, target_pose_3, PLANNING_GROUP)) {
    ...
    if (planAndExecuteCartesianPath(move_group, start_pose_5, target_pose_5, PLANNING_GROUP)) {
      ...
      RCLCPP_ERROR(LOGGER, "gripper opening failed.");
    }
