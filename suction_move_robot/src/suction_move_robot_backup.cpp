    ...
    while (std::getline(file, line)) {
        ...
        if (row.size() > 0 && row[0] == std::to_string(line_number)) {
            ...
            try {
                ...
                rclcpp::Node::SharedPtr node = rclcpp::Node::make_shared("suction_move_robot");
...
int main(int argc, char** argv)
{
  ...
  while (attempts <= 8) {
      target_pose_2a = convertAndPrintGraspDataToBaseLink("/home/moniesha/suctionnet-baseline/neural_network/example_data/save_data/test_novel/scene_0160/realsense/suction/0000.csv", tf_buffer, attempts);
      ...
      // Check if the angle is at least 30 degrees
      if ((90-angle_deg) < 40.0) {
      ...
      // Additional filtering conditions for position components
      if (!(target_pose_2a.position.x > -0.31 && target_pose_2a.position.x < 0.014) ||
      !(target_pose_2a.position.y > 0.55 && target_pose_2a.position.y < 0.903)) {
      RCLCPP_WARN(LOGGER, "Attempt %d rejected: Position constraints not met. Trying next pose...", attempts);
      attempts++;
      continue; // Skip the rest of the loop and try the next attempt
      }

      // Desired distance to move backward along the Z-axis
      double distance1 = 0.150; // Adjusting gripper length (0.165 for rg2 baseline gripper, 0.255 (initial) , then 0.245 for fin-ray gripper)
  ...
  if (planAndExecuteCartesianPath(move_group, start_pose_2, target_pose_2, PLANNING_GROUP)) {
    ...
    planAndExecuteCartesianPath(move_group, start_pose_0, target_pose_6, PLANNING_GROUP); 
    return 1;
  }
